...
## My Deals table
- Columns: Deal · Status · Interested buyers · Intros · Deadline · Next Steps · Update. Main table = live deals incl. Hold; Obsolete/Lost/Won/Trade Broken go to a collapsed "Archived deals (N)" <details> below. Summary: pipeline total = sizes of exactly the deals counted in "N live" (model live_rows); "in motion" = active live deals only; "introduced total" = every company across all rows incl. archived (each company once).
...
- Next Steps: never a paperwork chip. Red only for "Extend deadline →" (deadline 4006402 past); archived Lost/Obsolete/Trade Broken rows -> red "Reopen →" (never on Won rows); Provide deal terms / Nudge buyers / Complete deal terms are amber. Extend/Reopen link the deal update form.
...
- Page: h1 "Overview" + subtitle "<team name, else firm, else viewer> · updated <relative time of deals.json LastModified>". Order: tiles → Active intros → Open deals → Needs your attention → Track record. No active intros -> the card is replaced by one muted line under the tiles ("No introductions yet. Buyers are introduced once there's interest and paperwork is in place.") and Open deals comes next. No archived deals -> Track record is one muted "No closed deals yet." line.
- Tiles: Capital Raised (first, largest) · Live deals (= My Deals "N live") · $ in pipeline (= "Total in pipeline" = Σ sizes over exactly the Live-deals set) · Active intros (Σ non_terminal_count) · Won (count of won deals = Track-record won rows = My Deals "N won") · Buyers introduced (unique buyers behind intro_count) · Intro → won (Σ won_count / Σ intro_count).
...
- Needs your attention: My Deals' red Next Steps chips EXCLUDING Reopen (_my_deal_row_chip_html + RED_ACTION_CHIP_RE) plus one amber item per missing paperwork item on live-table rows (same links as the Status lines), else green "All paperwork in order, no deadlines past."
- Open deals: first 10 main-table rows; column header "Status". Active intros: 10 most recently updated rows of Active Intros' own set; buyer name only when disclosed (or admin edit), else "Buyer <code>". Track record: won deals + "N deals listed · N won · N lost/obsolete since <date>" + collapsed "All closed deals (N)".
