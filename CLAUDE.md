...
- Paperwork rule — ONE function, deal_paperwork_status (My Deals Visibility + Next Steps, company-page Deal Details card): in order only when BOTH (a) CEF 3796440 is Yes 6600515 or N/A 6600516 on ANY current team member (individual: the tenant) — team-level via _deal_id_status — and (b) the deal's Agent Agreement 3714334 contains 6354277 (sell-side signed; buy-side 6354274 / In-Process do NOT count). Missing -> "Not live · CEF required" and/or "Agent agreement required" (red). Nav badge uses the same team-level CEF ("CEF required — FINRA compliance").
