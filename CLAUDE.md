...
- Next Steps: paperwork missing -> one red chip per missing item (CEF form / agreement doc). In order -> red only for "Extend deadline →" (deadline 4006402 past); archived Lost/Obsolete/Trade Broken rows -> red "Reopen →" (never on Won rows); Provide deal terms / Nudge buyers / Complete deal terms are amber. Extend/Reopen link the deal update form.
...
- Page: h1 "Overview" + subtitle "<team name, else firm, else viewer> · updated <relative time of deals.json LastModified>". Order: tiles → Active intros → Open deals → Needs your attention → Track record.
- Tiles: Capital Raised (first, largest) · Live deals (= My Deals "N live") · $ in pipeline (= "Total in pipeline") · Active intros (Σ non_terminal_count) · Won (count of won deals = Track-record won rows = My Deals "N won") · Buyers introduced (unique buyers behind intro_count) · Intro → won (Σ won_count / Σ intro_count).
...
- Needs your attention: exactly My Deals' red Next Steps chips EXCLUDING Reopen (_my_deal_row_chip_html + RED_ACTION_CHIP_RE), else green "All paperwork in order, no deadlines past."
