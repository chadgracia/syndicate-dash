...
- Won intro = Intro Status Closed 7207587, override-aware via _resolve_intro_status (a buy deal at stage Won resolves to Closed — existing rule). Helper: _intro_is_won.
...
- Capital Raised = Σ _intro_amount (buy deal ticket max, else min, never native "value") over disclosed intros at Wired 7207583 or Closed, each intro once. Helper: _intro_counts_toward_raised. It is _company_buy_stats["raised"] (company-page "Total raised", My Deals raised line, Overview Capital Raised tile). The desk-wide admin headline (_raised_headline_stats) still uses its older closed-only rule.
