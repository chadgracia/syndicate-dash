...
- Write rights: teammates may add DEAL NOTES on another member's intro; status/milestones/flags/next steps/follow-up and deal-stage actions stay owner-only. Feature requests stay per individual.
...
- Deal note: Dynamo tenant=<deal's owning seller email (_tenant_email_for_deal)>, sk="intro#<deal_id>". About one intro. Write: POST ?action=update_intro {notes} (owner or a teammate, disclosed row only; admin: any).
